...
<div className={`bg-white relative flex  mt-24 w-fit m-auto rounded-3xl h-300px transition-all duration-[2000ms] ${animate ? 'translate-y-0 opacity-100' : 'translate-y-full opacity-0'}`}>
  <div className=' z-10 p-20 max-w-lg mx-auto  flex-1 '>
    ...
    <div className='flex flex-row items-center'>
      ...
      <span className={`text-3xl text-center font-semibold my-7 transition-all duration-[2000ms] ${animate ? 'translate-x-0 opacity-full' : '-translate-x-60 opacity-0'}`}>Sign In</span>
    </div>
  ...
  {error && <p className='text-red-500 mt-5'>{error}</p>}
      ...
      <div className={`z-10 p-10 max-w-lg mx-auto bg-black flex-1 rounded-3xl m-2 mr-2 text-white transition-all duration-[2000ms] ${animate ? 'translate-x-0 opacity-full' : 'translate-x-full opacity-0'}`} >
        ...
        <br/><h3 className='text-2xl'>🏠 Sign in to:</h3>
